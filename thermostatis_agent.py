...
class ActiveExteroception(ExteroceptiveAgent):
    ...
    def update_world(self):
        ...
        if not self.supress_action:
            self.velocity_action.append(self.aex_action[-1])


    def upd_aex_mu_d1(self):
        upd = -self.learn_r_aex * (self.aex_e_w_0[-1] / self.aex_s_w_0)
        upd *= self.dt
    ...
    def upd_aex_err_w_0(self):
        # my expected dynamics is light change should be explained
        self.aex_e_w_0.append(self.ex_sense[-1] - (-self.aex_mu[-1]))
        # self.aex_e_w_0.append(self.aex_mu_d1[-1] - (self.ex_sense[-1] - (-self.aex_mu[-1])))

    def upd_aex_err_z_0(self):
        """ error between sensation and generated sensation
        sensation: change in light
        generated by: change in change in temperature
        """
        ext_temperature_2 = self.temp_change_environment[-2] if len(self.temp_change_environment) > 1 else 0
        ext_temperature_change = self.temp_change_environment[-1] - ext_temperature_2
        # TODO need to convert to sense!

        # self.aex_e_z_0.append(self.ex_sense[-1] - (-self.mu_d2[-1]))
        # self.aex_e_z_0.append(self.ex_sense[-1] - (-ext_temperature_change))
        # mu is approximation of the external temperature change
        # self.aex_e_z_0.append(ext_temperature_change - self.aex_mu[-1])
        self.aex_e_z_0.append(self.temp_change_environment[-1] - self.aex_mu[-1])
    ...
    def plot_results(self):
        ...
        ax[1][0].plot(timeline, self.aex_mu_d1[1:])
