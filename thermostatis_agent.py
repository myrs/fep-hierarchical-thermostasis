...
class InteroceptiveAgent:
    ...
    def reset(self):
        ...
        self.temp_change = [0]
        # change in change in temp
        self.temp_change_d1 = [0]
        ...
        # overall light
        self.light_change = [0]
        # change in change of light
        self.light_change_d1 = [0]
    ...
    def update_world(self):
        ...
        if self.time == 50:
            # jump by 2 (to 2)
            temp_instant_change_of_constant = 2
        elif self.time == 100:
            # jump by 3 (to 5)
            temp_instant_change_of_constant = 3
        elif self.time == 150:
            # jump by -6 (to -1)
            temp_instant_change_of_constant = -6
        elif self.time == 200:
            # jump by -5 (to -6)
            temp_instant_change_of_constant = -5
        elif self.time == 250:
            # jump by +6 (to 0)
            temp_instant_change_of_constant = +6
        else:
            # stay at 0
            temp_instant_change_of_constant = 0

        temp_const_change = self.temp_const_change[-1] + temp_instant_change_of_constant
    ...
    def upd_velocity(self):
        ...
        velocity_change = self.velocity_change[-1]
        # with it's derivative (jerk)
        velocity_change += self.velocity_change_d1[-1]
        # velocity_change += self.velocity_change_d1[-1] * self.dt
        # velocity_change += self.velocity_change_d1[-1] * self.dt
        self.velocity_change.append(velocity_change)

        # update velocity
        velocity = 0.9 * self.velocity[-1]
        # velocity += self.velocity_change[-1]
        velocity += self.velocity_change[-1] * self.dt

        # TODO simulate friction
        # velocity = 0.9 * self.velocity[-1]
        # velocity = self.velocity[-1] -0.1 * self.velocity[-1] * self.dt
    ...
    def upd_temp_change(self):
        ...
        # self.temp_const_change[-1] += 1.0 * self.velocity[-1]
        # self.temp_const_change[-1] += 1.0 * self.velocity[-1] 
        self.temp_const_change[-1] += 1.0 * self.velocity[-1] * self.dt
        temp_change = self.temp_const_change[-1]
        temp_change += self.temp_change_action[-1]

        # how velocity and action changed the temperature
        # temp_change_d1 = 1.0 * self.velocity[-1] + self.temp_change_action[-1]
        temp_change_prev = self.temp_change[-2] if len(self.temp_change) > 1 else 0
        # divided by dt to get timestep
        # temp_change_d1 = (self.temp_change[-1] - temp_change_prev) / self.dt
        temp_change_d1 = (self.temp_change[-1] - temp_change_prev)

        self.temp_change_d1.append(temp_change_d1)
    ...
    def upd_light_change(self):
        # self.light_change_const[-1] += 1.0 * self.velocity[-1]
        self.light_change_const[-1] += 1.0 * self.velocity_change[-1] * self.dt
        light_change = self.light_change_const[-1]
        self.light_change.append(light_change)

        # how light change changed in time
        # TODO here the coefficient could be different
        # light_change_d1 = 1.0 * self.velocity_change[-1] * self.dt
        light_change_prev = self.light_change[-2] if len(self.light_change) > 1 else 0
        # divided by dt to get timestep
        # light_change_d1 = (self.light_change[-1] - light_change_prev) / self.dt
        light_change_d1 = (self.light_change[-1] - light_change_prev)

        self.light_change_d1.append(light_change_d1)
    ...
    def plot_results(self):
        ...
        diff = np.array(self.temp_const_change[1:]) + \
...
class ExteroceptiveAgent(InteroceptiveAgent):
    ...
    def generate_ex_sense_d1(self):
        ex_sense_d1 = self.light_change_d1[-1] + get_noise() * 0.1
        self.ex_sense_d1.append(ex_sense_d1)
    ...
    def update_world(self):
        ...
        light_change_const = self.light_change_const[-1] + light_change_instant
...
class ActiveExteroception(ExteroceptiveAgent):
    ...
    def update_world(self):
        ...
        if not self.supress_action:
            ...
            # OLD
            # velocity_change = upd
            # velocity_change = self.velocity_change[-1] + upd

            # # # OLD 2
            action_2 = self.aex_action[-2] if len(self.aex_action) > 1 else 0
            action_1 = self.aex_action[-1]
            upd = action_1 - action_2

            self.velocity_change.append(upd / self.dt)

            # self.velocity_change.append(upd)
            # self.velocity[-1] = self.aex_action[-1]
            # self.velocity[-1] = self.aex_action[-1] * self.dt
            # self.velocity[-1] = self.velocity[-1] + upd
            # self.velocity[-1] = 0
            # self.velocity.append(self.aex_action[-1])


            # # self.velocity_change_d1.append(self.velocity_change_d1[-1] + velocity_change)
            # # self.velocity_change_d1.append(self.velocity_change_d1[-1] + upd)

            # # self.velocity.append(self.aex_action[-1]) 
            # self.velocity[-1] = self.aex_action[-1]

            # velocity_change = self.velocity_change[-1]
            # velocity_change += self.aex_action[-1]
            # self.velocity_change.append(velocity_change)
            # self.velocity_change.append(self.aex_action[-1])


            # OLD 3
            # self.velocity_change.append(self.aex_action[-1])
            
            # velocity_change_d1 = self.velocity_change_d1[-1]
            # velocity_change_d1 += self.aex_action[-1]
            # self.velocity_change_d1.append(velocity_change_d1)
            ...
            # self.velocity_change_d1.append(0)
            # self.velocity_change_d1.append(self.aex_action[-1])
            # self.velocity_change_d1.append(self.velocity_change_d1[-1] + self.aex_action[-1])
    ...
    def upd_mu_d2(self):
        upd = -self.learn_r * (
            self.e_w_1[-1] / self.s_w_1
            # - self.aex_e_z_1[-1] / self.aex_s_z_1
            # + self.aex_e_z_1[-1] / self.aex_s_z_1
            )
        upd *= self.dt
    ...
    def upd_mu(self):
        upd = -self.learn_r * \
            (-self.e_z_0[-1] / self.s_z_0
            # - self.aex_e_z_1[-1] / self.aex_s_z_1
             # - self.aex_e_z_0[-1] / self.aex_s_z_0
             + self.e_w_0[-1] / self.s_w_0)
        upd += self.mu_d1[-2]
        upd *= self.dt
    ...
    def upd_aex_err_z_0(self):
        ...
        # self.aex_e_z_0.append(self.ex_sense[-1] - 1.0 * (-self.mu[-1] + self.temp_desire[-1]))
        # self.aex_e_z_0.append(self.ex_sense[-1] - 1.0 * (-self.mu_d1[-1]))
        self.aex_e_z_0.append(0)

    def upd_aex_err_z_1(self):
        self.aex_e_z_1.append(self.ex_sense[-1] - 1.0 * (-self.mu_d2[-1]))
    ...
    def upd_action(self):
        super().upd_action()
        # sensation change over action is always 1
        upd = -self.learn_r_aex * 1 * (self.aex_e_z_1[-1] / self.aex_s_z_1)
        # upd = -self.learn_r_aex * 1 * (self.aex_e_z_0[-1] / self.aex_s_z_0)
        upd *= self.dt
        aex_action = self.aex_action[-1] + upd
        # because it's a change in velocity
        # aex_action = upd
