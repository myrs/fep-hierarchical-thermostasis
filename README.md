...
While the model is shown to be useful, the problem of how an agent 'knows' what change of light corresponds to the change in the environment and what change of light corresponds to the change because it's actions remains to be addressed in the following steps. It it can be shown, that just adding *efference copy* of action, subtracting it from the light intensity in the exteroceptive layer inferring desired temperature solved the problem. But this would lead us to the closed loop system, that will accumulate error in time as the agent does not really has the exact information about it's action. 
